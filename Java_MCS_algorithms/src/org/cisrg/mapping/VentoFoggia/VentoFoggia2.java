...
import org.cisrg.mapping.VentoFoggia.VFState;
...
public final class VentoFoggia2 extends Pattern {
     ...
     */
    private VentoFoggia2(IAtomContainer query, AtomMatcher atomMatcher, BondMatcher bondMatcher, boolean substructure) {
        this.query = query;
    ...
    public static Pattern findSubstructure(IAtomContainer query, boolean anyBonds) {
        boolean isQuery = query instanceof IQueryAtomContainer;
        
        // XXX Modded by Edmund Duesbury, April 2015
        BondMatcher bm;
        if( anyBonds )
        	bm = BondMatcher.forAny();  // XXX Modded by Edmund Duesbury, August 2015
        else if( isQuery )
        	bm = BondMatcher.forQuery();
        else
        	bm = BondMatcher.forStrictOrder();
        
        return new VentoFoggia2(
        		query, 
        		isQuery ? AtomMatcher.forQuery() : AtomMatcher.forElement(),
        		bm,  // XXX Modded by Edmund Duesbury, April 2015
                true
        ); 
    }
    ...
    public static Pattern findIdentical(IAtomContainer query) {
        boolean isQuery = query instanceof IQueryAtomContainer;
        return new VentoFoggia2(query, isQuery ? AtomMatcher.forQuery() : AtomMatcher.forElement(),
