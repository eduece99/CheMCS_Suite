...
public final class McGregor2 {
     ...
     */
    public McGregor2(IAtomContainer source, IAtomContainer target, List<List<Integer>> mappings,
     ...
     * @param mappings
     */
    public McGregor2(IQueryAtomContainer source, IAtomContainer target, List<List<Integer>> mappings) {
    ...
    public void startMcGregorIteration(int largestMappingSize, Map<Integer, Integer> presentMapping)
        ...
        List<String> cTab1Copy = McGregorChecks2.generateCTabCopy(source);
        List<String> cTab2Copy = McGregorChecks2.generateCTabCopy(target);
        ...
        List<Integer> unmappedAtomsMolA = McGregorChecks2.markUnMappedAtoms(true, source, presentMapping);
        int counter = 0;
        int gSetBondNumA = 0;
        int gSetBondNumB = 0;
        ...
        List<Integer> unmappedAtomsMolB = McGregorChecks2.markUnMappedAtoms(false, target, presentMapping);
        ...
        McgregorHelper2 mcGregorHelper = new McgregorHelper2(dummy, presentMapping.size(), mappedAtoms,
    ...
    public void startMcGregorIteration(int largestMappingSize, List<Integer> cliqueVector,
        ...
        //find unmapped atoms of molecule A
        List<Integer> unmappedAtomsMolA = McGregorChecks2.markUnMappedAtoms(true, source, mappedAtoms, cliqueSize);
        ...
        //find unmapped atoms of molecule B
        List<Integer> unmappedAtomsMolB = McGregorChecks2.markUnMappedAtoms(false, target, mappedAtoms, cliqueSize);
        ...
        McgregorHelper2 mcGregorHelper = new McgregorHelper2(dummy, mappedAtomCount, mappedAtoms, localNeighborBondnumA,
    ...
    private int iterator(McgregorHelper2 mcGregorHelper) throws IOException {
        ...
        boolean furtherMappingFlag = McGregorChecks2.isFurtherMappingPossible(source, target, mcGregorHelper,
                isBondMatch());
    ...
    private void searchAndExtendMappings(Stack<List<Integer>> bestarcsCopy, McgregorHelper2 mcGregorHelper)
            throws IOException {
        int mappedAtomCount = mcGregorHelper.getMappedAtomCount();
        ...
        while (!bestarcsCopy.empty()) {
            ...
            List<String> cSetACopy = McGregorChecks2.generateCSetCopy(setNumA, cBondSetA);
            List<String> cSetBCopy = McGregorChecks2.generateCSetCopy(setNumB, cBondSetB);
            ...
            McgregorHelper2 newMH = new McgregorHelper2(noFurtherMappings, newMapingSize, newMapping, newNeighborNumA,
    ...
    private List<Integer> findMcGregorMapping(List<Integer> mArcs, McgregorHelper2 mcGregorHelper) {
        ...
        List<Integer> uniqueMapping = McGregorChecks2.removeRecurringMappings(currentMapping);
        return uniqueMapping;
    }

    private void setModifedArcs(McgregorHelper2 mcGregorHelper) {
        int neighborBondNumA = mcGregorHelper.getNeighborBondNumA();
        int neighborBondNumB = mcGregorHelper.getNeighborBondNumB();
        ...
        for (int row = 0; row < neighborBondNumA; row++) {
            for (int column = 0; column < neighborBondNumB; column++) {
                ...
                if (matchGAtoms(g1A, g2A, g1B, g2B)) {
                    ...
                    if (McGregorChecks2.isMatchFeasible(source, reactantBond, target, productBond, isBondMatch())) {
    ...
    private void partsearch(int xstart, int ystart, List<Integer> tempMArcsOrg, McgregorHelper2 mcGregorHelper) {
        int neighborBondNumA = mcGregorHelper.getNeighborBondNumA();
        int neighborBondNumB = mcGregorHelper.getNeighborBondNumB();
        ...
        if (tempMArcs.get(xstart * neighborBondNumB + ystart) == 1) {
            ...
            McGregorChecks2.removeRedundantArcs(xstart, ystart, tempMArcs, mcGregorHelper);
            int arcsleft = McGregorChecks2.countArcsLeft(tempMArcs, neighborBondNumA, neighborBondNumB);
    ...
    private void startsearch(McgregorHelper2 mcGregorHelper) {
        int neighborBondNumA = mcGregorHelper.getNeighborBondNumA();
        int neighborBondNumB = mcGregorHelper.getNeighborBondNumB();
    ...
    private void setArcs(int xIndex, int yIndex, int arcsleft, List<Integer> tempMArcs, McgregorHelper2 mcGregorHelper) {
        int neighborBondNumA = mcGregorHelper.getNeighborBondNumA();
        int neighborBondNumB = mcGregorHelper.getNeighborBondNumB();
        do {
            yIndex++;
    ...
    private void popBestArcs(int arcsleft) {
        if (arcsleft > bestarcsleft) {
            McGregorChecks2.removeTreeStructure(first);
    ...
    private void extendMapping(int xIndex, int yIndex, McgregorHelper2 mcGregorHelper, List<Integer> additionalMapping,
