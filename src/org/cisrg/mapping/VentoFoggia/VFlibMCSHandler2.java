...
import org.cisrg.mapping.McGregor2;
...
public class VFlibMCSHandler2 extends AbstractMCSAlgorithm implements IMCSBase {

	// XXX  Removed all "static" variable declarations to make the class thread-safe
    ...
    private final static ILoggingTool                LOGGER         = LoggingToolFactory
            .createLoggingTool(VFlibMCSHandler2.class);

    /**
     * Constructor for an extended VF Algorithm for the MCS search
     */
    @TestMethod("setMCSAlgorithm")
    public VFlibMCSHandler2() {
    ...
    private void searchMcGregorMapping() throws CDKException, IOException {
        ...
        for (Map<Integer, Integer> firstPassMappings : allMCSCopy) {
            ...
            McGregor2 mgit = null;
            if (queryMol != null) {
                mgit = new McGregor2(queryMol, mol2, mappings, isBondMatchFlag());
            } else {
                if (countR > countP) {
                    mgit = new McGregor2(mol1, mol2, mappings, isBondMatchFlag());
                } else {
                    tMapping.clear();
                    mgit = new McGregor2(mol2, mol1, mappings, isBondMatchFlag());
                    ropFlag = false;
