        ...
        for (int c = 0; c < mappingSize; c++) {
            if (mapped_atoms.get(c * 2 + 1).equals(indexJ)) {
                ...
                if (cTab2Copy.get(atomIndex * 4 + 3).compareToIgnoreCase("X") == 0) {
                    ...
                    McGregorChecks2.changeCharBonds(indexJ, signArray[counter], target.getBondCount(),
                            target, cTab2Copy);
                    int cor_atom = McGregorChecks2.searchCorrespondingAtom(mappingSize, indexJ, 2, mapped_atoms);
                    //Commented by Asad
                    McGregorChecks2.changeCharBonds(cor_atom, signArray[counter], newNeighborNumA,
        ...
        for (int c = 0; c < mappingSize; c++) {
            if (mapped_atoms.get(c * 2 + 1).equals(indexI)) {
                ...
                if (cTab2Copy.get(atomIndex * 4 + 2).compareToIgnoreCase("X") == 0) {
                    ...
                    McGregorChecks2.changeCharBonds(indexI, signArray[counter], target.getBondCount(),
                            target, cTab2Copy);
                    int cor_atom = McGregorChecks2.searchCorrespondingAtom(mappingSize, indexI, 2, mapped_atoms);
                    McGregorChecks2.changeCharBonds(cor_atom, signArray[counter], newNeighborNumA,
                            newINeighborsA, newCNeighborsA);
//                                changeCharBonds(cor_atom, signArray[counter], query.getBondCount(), query, cTab1Copy);
                    counter++;
    ...
    private boolean unMappedAtomsEqualsIndexI(
        ...
        for (int c = 0; c < newMappingSize; c++) {
            if (new_Mapping.get(c * 2 + 1).equals(indexJ)) {
                ...
                if (cTab2Copy.get(atomIndex * 4 + 3).compareToIgnoreCase("X") == 0) {
                    step1(atomIndex, counter);
                    McGregorChecks2.changeCharBonds(indexJ, signArray[counter], setNumB, i_bond_setB, cTab2Copy);
                    int cor_atom = McGregorChecks2.searchCorrespondingAtom(newMappingSize, indexJ, 2, new_Mapping);
                    McGregorChecks2.changeCharBonds(cor_atom, signArray[counter], newNeighborNumA,
                            newINeighborsA, newCNeighborsA);
                    counter++;
        ...
        for (int c = 0; c < newMappingSize; c++) {
            if (new_Mapping.get(c * 2 + 1).equals(indexI)) {
                ...
                if (cTab2Copy.get(atomIndex * 4 + 2).compareToIgnoreCase("X") == 0) {
                    ...
                    step3(atomIndex, counter);
                    McGregorChecks2.changeCharBonds(indexI, signArray[counter], setNumB, i_bond_setB, cTab2Copy);
                    int cor_atom = McGregorChecks2.searchCorrespondingAtom(newMappingSize, indexI, 2, new_Mapping);
                    McGregorChecks2.changeCharBonds(cor_atom, signArray[counter], newNeighborNumA,
                            newINeighborsA, newCNeighborsA);
                    counter++;
                } else {
